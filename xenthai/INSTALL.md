...
Five facts make it exact, all measured on real sessions.

**The `if` pattern must not begin with the command name.** A session invokes the tool by absolute
path (`node "/root/.claude/plugins/.../tools/journal-sync.mjs" --emit …`), which
`Bash(node * journal-sync.mjs --emit*)` does not match — and an `if` that does not match fails by
doing nothing at all, so the upload silently falls back to costing tokens.
`Bash(*journal-sync.mjs --emit*)` matches both spellings.

**The emit command is the whole Bash call.** What the hook copies is that call's entire stdout, so
anything else on the line ends up inside the file: a `date`, an `echo`, a second command after `&&`.
Measured, a one-line banner put 16 extra bytes into an uploaded revision — `--receipt`'s size check
caught it, which is the second reason that check exists. Run the emit line by itself, with nothing
before it, nothing after it, and no redirection.

**`server`** is the connector's name as the hooks see it, which arrives as `mcp_server.name` in any
hook input for a connector call (`Google_Drive` on one host, a UUID on another) — read it there,
never guess it. **The `title`** is the Bash call's `description`, so the session runs the emit
command with the file's exact name as the description. And **a Bash call's stdout reaches the hook
without its final newline**, which is why `--emit` prints the file without it and the hook's `"\n"`
puts it back; the size check in `--receipt` is what proves the result either way. That same output is
capped, which is why `--emit` refuses to print anything over the transport budget rather than letting
a truncated file look delivered.
...
bytes generated by the model, about 45 seconds**, and revision 012, one file, 16,453 bytes, about 46
seconds — against six minutes for revision 009 through the connector by hand. The state file beside
them was 887 bytes.
