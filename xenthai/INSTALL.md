...
truncating a file while reporting success, and the size is how that is caught. The next session
starts with `--restore --from <dir>` after downloading each month's highest whole-month revision and
every delta after it. `doctor` fails while anything is owed, and `report` and `opportunities` say
when the history they need is in the store rather than on this machine.
...
with the bytes copied from a Bash command's output instead, in about a second. It is written once
per company into `<company root>/.claude/settings.json` (or `settings.local.json`), because the
connector's name and the folder id are per host and per store:
...
folder id are its constraint. Claude Code loads a settings change during the session; no restart. On
a OneDrive store the `server`, `tool` and `input` keys are the ones `company-new` recorded in
`store.tools`.
