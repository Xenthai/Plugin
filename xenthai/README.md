...
- **The repository is the catalogue.** Its `marketplace.json` sits at the root and points at
...
Point it at the checkout's **root**, not at this directory: the catalogue is one level up, and
`./xenthai` resolves from there the same way it resolves on GitHub. The install line afterwards is
the same one.

There is deliberately no second catalogue for development. Claude Code keeps one marketplace per
...
```
../.claude-plugin/  the marketplace manifest, at the repository root, pointing at ./xenthai
