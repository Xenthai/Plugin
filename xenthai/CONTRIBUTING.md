...
`capabilities/method/method.json` and `capabilities/method/tables/*.md` are generated by the Web
repository's `pnpm --filter method export` and are never edited by hand here — a hand edit is
overwritten by the next export and carries no authority, since the playbook is normative for
method (rubrics, levels, phases, store layout) and this directory is its vendored copy. Every
number a doctrine file or a skill cites — a weight, a threshold, a band, a level — comes from this
vendored data, never from a value re-typed by hand; link to the table instead of restating the
numbers.
