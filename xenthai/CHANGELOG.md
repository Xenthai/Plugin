...
oversized-emit refusal fires. **Revision 012 went up as one file of 16,453 bytes in about 46
seconds, with no byte generated by the model**, against six minutes for revision 009 by hand.

### Fixed — a test that depended on how long the sandbox's paths were

- **`test/journalstate.test.mjs`'s oversized-emit case passed here and failed in a container.** A
  journal row carries the absolute path it touched, so rows are longer there; the 40 rows the case
  writes crossed the shard budget, staging split them into parts, and the single-file `name` it
  emitted was `null`. `--emit null` exits 2 like the refusal does, with a different message, so
  every other condition held and only the message assertion failed. The case now emits whichever
  file staging produced and derives the budget from that file's real size, and it checks the state
  file's size at revision 1 against revision 12 rather than against a ratio that moves with the rows.

### Added — the emit command is the whole Bash call

- `INSTALL.md` §5b and the `doctor` skill now say to run the emit line **alone**: the hook copies
  that call's entire stdout, so a `date`, an `echo` or a second command after `&&` lands inside the
  uploaded file. Measured, a banner put 16 extra bytes into a revision — caught by `--receipt`'s
  size check, which is the second failure that check has caught that nothing else would have.

## [0.6.2] - 2026-09-20

Three findings from the first run with the transport hook actually firing. **The measurement it was
built for**: revision 011, one file, 12,260 bytes, **no byte generated by the model, about 45
seconds** — against six minutes for revision 009 uploaded by hand through the connector.

### Fixed — two ways the transport failed quietly

- **The hook's `if` pattern never matched.** `Bash(node * journal-sync.mjs --emit*)` does not match
  the absolute-path invocation a cloud session makes, and a pattern that does not match fails by
  doing nothing: the hook stayed silent and every byte went back through the model, with nothing
  anywhere saying so. `INSTALL.md` §5b now carries `Bash(*journal-sync.mjs --emit*)`, which matches
  both spellings, and says the settings file belongs in the directory the session **started in** —
  never in the company folder, which is not a settings location and was never read.
- **The state file was the whole local receipt**, so at eleven revisions it had grown to 50 KB and
  reached the store at 60% of itself: a hook is handed a command's output already truncated, with
  no error. What travels now is the head revision alone — a few hundred bytes whatever the month's
  history looks like — so it cannot outgrow the channel again. `--receipt` prints its exact byte
  count to compare against the store's, and **`--emit` refuses outright to print anything over the
  transport's budget** rather than let a truncated file look delivered. A truncated state file was
  already refused on the far side, since half a JSON document does not parse.

## [0.6.1] - 2026-09-20

0.6.0 was exercised in the environment it was written for — a cloud container, the personal store —
and the delta held: `--stage` named 2 files where 29 had been named, `--emit` came back byte-exact,
`--receipt` refused 19,842 bytes against a 19,843-byte file and named it. Two things that run found
were not slowness, and one of them was corrupting histories.

### Fixed — the revision chain survives the machine

- **The chain's state travels to the store.** `journal/sync/<month>.json` — which revision came last
  and how many rows it settled — lived only on the local disk. On an ephemeral binding that disk is
  destroyed, so the next session read "no revisions", proposed `rev-001`, and uploaded it into a
  folder that already held one: two different files with one name, and a month that can no longer be
  rebuilt. `--receipt` now names `<YYYY-MM>.sync.rev-<NNN>.json` to upload beside the rows (the head revision alone,
  `--emit` prints it, no receipt of its own — the name carries the revision it records).
- **`--adopt-state <file>` resumes a chain from that file alone**, with no row of history
  downloaded. `--restore` still exists and still answers the other question — it brings the rows
  back, which costs every revision file — so the two are no longer the same choice. Adopting is
  refused when this machine is already in the chain, by receipt or by rows, because counting its own
  history twice is the failure that looks like success.
- **`--stage` refuses a month's first revision on an ephemeral binding** unless `--first-revision`
  says the month was never uploaded (exit 4). From such a machine "I have no receipt" and "nobody
  has uploaded this month" are the same observation and have opposite consequences, so the tool
  stops rather than guessing, and names the state file to look for. `doctor`'s `sync` line says the
  same thing where it fails.
- **`RECEIPT_SCHEMA` 3.** An entry gains `local` — how much of *this machine's* file it settled —
  and a `chain` digest linking it to the revision before. A month whose history is carried records
  `digest: null` rather than hashing a tail and calling it the month; `--restore` recomputes the
  true month digest once it holds every file. Schema 1 and 2 receipts read unchanged.
- `--restore` compares a carried machine's file where it actually sits in the month, instead of
  refusing it for ever as holding rows the store does not have.
- **`test/journalstate.test.mjs`**: the fresh-container round trip, the refusal and its override,
  double-count refusals, the state file emitted byte-exact, and restore over a carried month.

### Fixed — where the transport hook lives

- 0.6.0 said the hook goes in `<company root>/.claude/settings.json`. Claude Code reads hooks from
  the user's settings and from the **session's own project** directory, so a settings file in a
  company folder that is not that directory is never read — it was written in one session and the
  next started without it. `INSTALL.md` §5b now says where it goes, and says plainly that on an
  ephemeral binding it is one `Write` per session or the tokens are paid: it cannot move into
  `plugin.json`, because `server`, the folder id and the tool name are per host and per store and an
  `mcp_tool` hook substitutes only `${path}` values from its own event — no file, no environment
  variable, no plugin option. The byte path itself was verified and is unchanged.

## [0.6.0] - 2026-09-19

Uploading a month's revision cost the session most of an hour and generated journal rows of its
own, because every revision was the whole month re-split into parts whose names all changed. And
the receipt that settled it never looked at the store's copy, so a file that arrived truncated would
have been recorded as delivered. Measured on the personal store's `2026-09.rev-009`: 29 parts, one
typed at ~13,000 tokens per 20 KB, 33 new rows owed afterwards. See `DECISIONS.md` #21d.

### Changed — journal revisions are deltas, and the receipt checks the store

- **Every revision after the first is a delta.** `--stage` freezes only the rows since the last
  receipt into `<YYYY-MM>.rev-<NNN>.delta-after-<R>.jsonl`, where `<R>` is the row count the previous
  receipt settled; `rev-001` is still the whole month, and a whole-month revision above deltas
  becomes the new base. A typical sync is one file. The receipt keeps recording the whole month's
  rows and digest, so `--check`, `doctor`'s `sync` line and the divergence rule are unchanged.
- **`--receipt` takes `<id>:<size>` per file and refuses a size that differs from the staged
  bytes.** The size is what the connector reports for the file it holds; a bare id is refused with
  the sentence saying where the size comes from. `RECEIPT_SCHEMA` is 2: entries carry `kind`,
  `after`, the uploaded bytes and the confirmed size per file. Schema 1 entries read as whole-month.
- **`--restore` chains the highest whole-month revision with every delta after it**, refusing by
  name a missing revision, a delta whose base disagrees with the rows before it, and a file that is
  not whole JSON rows — which is how a truncated download is told from a short one with no receipt
  at hand. Folders holding only the old whole-month spelling restore exactly as before.
- The months already in stores under the old spelling are not migrated or re-uploaded.

### Added — the transport hook, and a second store provider

- **`--emit <file>`** prints a staged file's bytes, minus the final newline, and nothing else. It
  exists for a `PostToolUse` hook of type `mcp_tool` that calls the session's connector with
  `"textContent": "${tool_response.stdout}\n"`, so the file reaches the store in about a second with
  no byte generated by the model. The byte path was verified end to end in a cloud session: 19,973
  bytes, `fileSize` equal, SHA-256 equal. Where the hook has to live, and what that costs on a
  machine whose disk does not survive, is `INSTALL.md` §5b — corrected in 0.6.1.
- **`store.kind` declares the provider: `drive` (default) or `onedrive`.** An unrecognised value is
  refused as unbound, and `doctor` says so. The Microsoft 365 connector's `sharepoint_*` write
  methods join the guard's veto and its read-only methods join the journal's skip list, as a union,
  so an unbound OneDrive write is refused today; `driveId`, `itemId` and `uri` are recorded as
  targets. A OneDrive root may be a Graph path; a URL is still refused. What the plugin cannot know
  in advance about that connector — the create tool's parameter names, its size ceiling — is
  recorded in `store.tools` by `company-new` after reading the connector's schema in the session.
- **`test/journaldelta.test.mjs`**: delta shape and round trip, a base mismatch refused at receipt
  time, a chain restored byte-identical, a missing revision, a wrong base and a truncated delta each
  refused by name, the old spelling and a rebase restored, a delta split into parts, and `--emit`.

### Fixed — a sentence that was too broad

- `DECISIONS.md` #21, `MCP.md` and `CONTROLS.md` §1b said a hook cannot upload. That holds for a
  `command` hook and for `SessionEnd`; it does not hold for an `mcp_tool` hook on `PostToolUse`. The
  three now say which, and the upload's control tier is stated accordingly.
