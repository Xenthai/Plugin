---
name: company-offer
description: Capture what a company sells and on what terms — products with stock and lead times, services with deliverables and capacity, and the payment, warranty, cancellation and CFDI terms, including the discount limit and who authorises exceeding it. Use when 01-oferta/OFERTA.md, 01-oferta/PRODUCTOS.md or 01-oferta/SERVICIOS.md are pending, when quoting or invoicing is about to be mapped or automated, or when a price must appear in public copy. For positioning use social-identity; for how the quote gets produced use process-map.
---

# Company offer — what is sold, and on what terms

One session. Deliverables: `01-oferta/OFERTA.md`, plus `01-oferta/PRODUCTOS.md` and `01-oferta/SERVICIOS.md` for whichever the
...
Before asking anything, check `01-empresa.md` for a price list, a catalogue, a recent real quote, a
...
| --- | --- | --- |
| `01-oferta/OFERTA.md` | Terms, how a price is built, the discount limit, what is never sold | These apply to everything the company sells; stating them once is what keeps them from drifting |
| `01-oferta/PRODUCTOS.md` | Catalogue with SKU, stock, supplier, lead time | **A product runs out.** Stock and lead time decide whether a quote can be promised |
| `01-oferta/SERVICIOS.md` | Catalogue with deliverable, duration, who executes, capacity | **A service saturates.** Capacity, not stock, is the constraint |

Fill `01-oferta/OFERTA.md` §1 first — the table saying whether the company sells products, services or both.
...
2. **Who can execute each service, and how many at once.** If one person is the only one who can
   ...
   not only an operational one — cross-reference it in `01-personas.md`.
...
node "${CLAUDE_PLUGIN_ROOT}/tools/journal.mjs" --event phase_end --capability company \
  ...
  --target "01-oferta/OFERTA.md"
```

Append what each person said to `04-evidencia/ENTREVISTAS.md` — prices and terms stated from memory are
...
- **`01-oferta/OFERTA.md` §1 is still pending.** Nothing else in these files means anything until it says
...
- **Prices exist only in someone's head.** Record them as client-reported, say so in
  `04-evidencia/ENTREVISTAS.md`, and do not let them into public copy until a document or an authorised person
  confirms them.
