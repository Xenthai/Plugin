...
description: Audit a company's current public presence before anything is produced for it — every account that exists including the ones nobody remembers, dated observations per platform, contradictions between channels, whether paid advertising is already running, regulatory exposure visible from outside, and a named competitor set observed the same day. Use at the very first social session, before identity or voice, whenever PRESENCE.md is absent or its fields are pending, when a company asks how it is doing on social, when an agency's benchmark deck needs answering, or when a before-and-after will later be claimed. Produces PRESENCE.md, which is append-only. For positioning and brand documents use social-identity; for the editorial plan use social-plan; for company-wide operational measurement use baseline.
---
