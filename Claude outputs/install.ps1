...
# An unattended install has nobody to answer a prompt. Windows PowerShell will drop into its
# debugger on some trapped errors inside a loop and print a [DBG] prompt into a log nobody is
# watching, which reads as a hang and hides whatever came after it. Turned off explicitly rather
# than hoped about, along with the progress bars that corrupt a redirected transcript.
Set-PSDebug -Off
...
<#
    The failures worth retrying, all of them one thing wearing three names.

    Windows reports "a file in here is open, so I will not move or delete this directory" as EPERM
    on a rename, EBUSY on a remove, and occasionally EACCES or ENOTEMPTY, and which one surfaces
    depends on where in the install the antivirus scanner happened to be holding its handle. Any
    filter that names only EPERM leaves the operator staring at EBUSY being reported as permanent
    when one more attempt would have worked.
#>
$Transient = 'EPERM|EBUSY|EACCES|ENOTEMPTY|Failed to finalize'
...
function Invoke-WithRetry {
    ...
    for ($i = 1; $i -le $Max; $i++) {
        ...
        if ($output -notmatch "$Transient|Failed to install|Failed to add") {
            if ($i -gt 1) { Write-Ok "$Label (succeeded on attempt $i of $Max)" }
            else          { Write-Ok  $Label }
            return $true
        }

        if ($output -notmatch $Transient) {
...
foreach ($key in $targets) {
    ...
    <#
        Proof, and `details` is not proof. `plugin details` resolves a plugin from the marketplace
        catalogue whether or not it is installed — it will report all 21 skills for a plugin that
        was never installed at all, which is exactly how an install that failed on EBUSY got
        reported as complete. `plugin list` reads the store's own record, and that record is the
        only thing that decides whether a session loads anything. Ask the store, then ask for the
        count.
    #>
    $record = (& claude plugin list --json @($s.Flag) 2>&1 | Out-String)
    if ($record -match [regex]::Escape($PluginId)) {
        $details = (& claude plugin details $PluginName @($s.Flag) 2>&1 | Out-String)
        if ($details -match 'Skills \((\d+)\)') { Write-Ok "installed, $($Matches[1]) skills" }
        else                                      { Write-Ok "installed" }
    } else {
        Write-Fail "the CLI reported success but $PluginId is not in this store's record"
        $failed += "$($s.Label): not recorded"
    }
    Write-Host ''
}
